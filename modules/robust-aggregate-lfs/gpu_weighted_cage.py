...
def probability_s_given_y_l(pi, s, y, l, k, continuous_mask, weights, ratio_agreement=0.85, model=1,
                            theta_process=2):
    ...
    for i in range(k.shape[0]):
        ...
        # BUGS: Fixed bugs in shape
        try:
            # if len(s.shape) == 1:
            #     s = s[None, :]
            # if len(l.shape) == 1:
            #     l = l[None, :]
            probability *= (torch.exp(m.log_prob(s[:, i].double())) * l[:, i].double() +
                            (1 - l[:, i]).double()) * continuous_mask[i] + (1 - continuous_mask[i])
